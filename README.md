...
campus_life posts are short, 3 to 9 lines each, and every one stays on a single topic even at its longest. Indexing the corpus with the starter's fixed 800-character window already produced 88 chunks from 88 documents, meaning it never actually split anything, since almost no post reaches 800 characters. So the starter's behavior was already "one post = one chunk" by accident. I made that explicit on purpose instead of relying on it as a side effect: split_documents in chunker.py now just cleans and returns each whole document as its own chunk, since splitting further would only break a single self-contained post into fragments that make less sense on their own.
